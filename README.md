...
문서 분석은 외부 API 없이 제목·구역 경계와 명시적인 의도 문장을 사용합니다. 목적, 기간, 최종목표, 연구내용, 성과물은 해당 구역의 여러 줄을 수집하며, 다음 제목이나 파일 경계에서 중단합니다. 명시 항목을 구역 근거보다, 구역 근거를 문맥 추정보다 우선합니다. 같은 우선순위에서는 문서 순서상 첫 근거를 선택합니다. 키워드 언급만으로 값을 채우지 않으며 근거가 없으면 `확인 필요`를 유지합니다.

기존 `fields.*.value` / `evidence`, `sourceFiles`, `textLength`, `warnings` 구조를 유지하고 필드에 다음 정보를 추가합니다.

- `confidence`: 규칙 기반 근거 강도(명시 항목 0.95, 구역 0.9, 문맥 0.7, 누락 0). 정답 확률이나 적격성 판정이 아닙니다.
- `evidenceType`: `explicit_label`, `section`, `contextual`, `missing`.
- `evidenceDetails`: 원문 텍스트, 추출 텍스트 줄 번호, 파일명(API), 확인 가능한 PDF 페이지 번호.
- `section`: 근거가 속한 제목(있는 경우).

PDF 페이지는 `pdf-parse`의 페이지 텍스트에서만 얻습니다. 페이지 번호를 텍스트 길이로 추정하지 않습니다. 파싱 실패나 이미지 전용 PDF에서는 근거를 만들어내지 않으며 수동 확인이 필요합니다. OCR은 제공하지 않습니다. 다단 편집이나 복잡한 표의 읽기 순서는 추출기의 한계가 있으며, 인식하지 못한 일반 텍스트 제목과 모호한 문장은 원문 대조가 필요합니다. 기존 문자열 반환 함수 `extractFile`도 유지합니다.
